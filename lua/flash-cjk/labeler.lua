...
---@field pool_set table<string, boolean>
---@field used_labels table<string, boolean>
---@field used table<string, string> pos id -> reused label
---@field next_label integer monotonic pool index
...
function M.new(state, langs, filter_keys, priority)
	...
	if priority and #priority > 0 then
		...
		self.lang_ranks = {}
		for rank, code in ipairs(priority) do
			self.lang_ranks[code] = rank
		end
...
-- each enabled engine that produces a spelling of `text` extending
...
function M.match_langs(text, prefix, langs)
	...
	if first ~= nil and first < 128 then
		-- engines pass ASCII through unchanged, so their spellings
		-- carry no language information for such spans
	...
	local function extends(strs)
		for _, spelling in ipairs(strs) do
			if string.sub(spelling, 1, #prefix) == prefix then
				return true
			end
		end
		return false
...
function M:attribution(m, prefix, line_cache)
	...
	local cache_key = m.win .. ":" .. m.pos[1]
	local line = line_cache[cache_key]
	if line == nil then
		local lines = vim.api.nvim_buf_get_lines(
			vim.api.nvim_win_get_buf(m.win),
			m.pos[1] - 1,
			m.pos[1],
			false
		)
	...
	-- include the character right after the match, as the Rust
...
function M:sort_by_priority(matches)
	...
	for i, match in ipairs(matches) do
		...
		local langs = self:attribution(match, clean, line_cache)
		if langs then
			for _, code in ipairs(langs) do
				local lang_rank = self.lang_ranks[code]
				if lang_rank and lang_rank < rank then
					rank = lang_rank
				end
			end
		end
		decorated[i] = { rank = rank, index = i, match = match }
	end
...
function M:skip(win, labels)
	...
	-- cumulative union of every predicted next letter, filtered against
	-- the label pool once
	...
	for _, match in ipairs(self.state.results) do
		if match.win == win then
			...
			local pred = preds
				and preds[string.format("%d:%d:%d", match.pos[1], match.pos[2], match.end_pos[2])]
			if pred then
				for i = 1, #pred.text do
					local char = string.sub(pred.text, i, i)
					skip_set[ignorecase and char:lower() or char] = true
				end
			else
				local strs = self:match_strs(line, start_col, end_col, langs)
				for _, spelling in ipairs(strs) do
					local char = string.sub(spelling, prefix_len + 1, prefix_len + 1)
					if char ~= "" then
						skip_set[ignorecase and char:lower() or char] = true
					end
				end
	...
	return vim.tbl_filter(function(char)
		return not skip_set[ignorecase and char:lower() or char]
	end, labels)
end
...
function M:reset()
	...
	for _, label in ipairs(self.state:labels()) do
	...
	-- pool bookkeeping for the monotonic assignment pointer: valid()
	-- must still reject labels removed by skip() this keystroke
	self.pool_set = {}
	for _, label in ipairs(self.labels) do
		self.pool_set[label] = true
	end
	self.used_labels = {}
	self.next_label = 1
	for _, match in ipairs(self.state.results) do
		if match.label ~= false then
			match.label = nil
		end
	end
end
...
-- Visible, labelable matches in label-issuance order: current window
-- first, then other windows, each by position or distance.
function M:filter()
	---@type Flash.Match[]
	local visible = {}
	...
	for _, match in ipairs(self.state.results) do
		local hide = (target and match.pos == target.pos)
			and not self.state.opts.label.current
			and match.win == self.state.win

		-- only the first match in each fold
		if not hide and match.fold then
	...
	table.sort(visible, function(a, b)
		...
		if a.win ~= b.win then
			local rank_a = a.win == self.state.win and 0 or a.win
			local rank_b = b.win == self.state.win and 0 or b.win
			return rank_a < rank_b
		end
		if use_distance then
			local function cell(pos)
				return pos[1] * vim.go.columns + pos[2]
			end
			return math.abs(cell(from) - cell(a.pos)) < math.abs(cell(from) - cell(b.pos))
		end
