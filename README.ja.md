...
`priority` はラベル割り当ての順序を言語で決めます。リストの前方の言語で到達できる一致が早いラベルから先に受け取り（複数言語で解釈できる一致は最も優先度の高い言語に帰属します）、主言語の目標がより少ないラベルキーでジャンプできます。一致セットとジャンプのセマンティクスは変わらず、未設定なら位置順序のままです。
