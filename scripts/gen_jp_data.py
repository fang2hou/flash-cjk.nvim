...
OUT = Path(__file__).resolve().parent.parent / "lua" / "flash-cjk" / "lang" / "ja" / "data.json"
...
def char_class(chars: set[str]) -> str:
    return "[" + "".join(sorted(chars)) + "]"


def build_tables(readings, singles, combos) -> dict:
    p3: dict[str, str] = {}
    for key in sorted(set(singles[3]) | set(combos)):
        # single-char classes merged with two-char youon atoms
        parts: list[str] = []
...
def emit(readings, singles, combos) -> str:
    tables = build_tables(readings, singles, combos)
    return json.dumps(tables, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
