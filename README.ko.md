...
|                  | vim-regex |        Rust |            가속 |
| ---------------- | --------: | ----------: | --------------: |
| 전체 평균        |   8.23 ms | **0.97 ms** |            8.5× |
| 전체 p95         |   29.1 ms | **4.35 ms** |            6.7× |
| 일부 고부하 상황 |           |             | **최대 약 27×** |

vim-regex의 부하가 가장 큰 시나리오에서는 일부 세 언어 조합의 평균이 30 ms에 가깝고 p95는 243 ms에 달합니다. Rust 매처는 주로 이런 극단적인 상황에서 입력 지연을 낮추기 위해 사용합니다.
...
| **전체(1,050개 윈도우)**    |    **8.23 ms** | **0.97 ms** | **8.5×** |   **29.1 ms** | **4.35 ms** |
